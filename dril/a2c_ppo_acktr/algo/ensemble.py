...
def Ensemble (uncertainty_reward=None, device=None, envs=None,\
    ...
    if os.path.exists(ensemble_model_path):
        ...
        for ensemble_epoch in range(num_ensemble_train_epoch):
            ...
            if ensemble_test_loss < best_test_loss:
                print('Ensemble model has improved')
                best_test_loss = ensemble_test_loss
                best_test_params = copy.deepcopy(ensemble_policy.state_dict())
                patience = 20
            else:
                patience -= 1
                print('Ensemble model has not improved')
                if patience == 0:
                    print('Ensemble model has not improved in 20 epochs, breaking')
                    break
        # best_test_params = copy.deepcopy(ensemble_policy.state_dict())
